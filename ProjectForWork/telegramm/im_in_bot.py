...
SERVICE_ACCOUNT_FILE = '/home/vlados/bot_for_im_in/keys/feisty-current-452513-v7-4247448ae8c9.json'
if not SERVICE_ACCOUNT_FILE:
    print("Ошибка: Не найден путь к файлу credentials.json в переменных окружения.")
    exit()

# ID таблицы Google Sheets
SPREADSHEET_ID = '1jH_54N-i0nc3v7w07E-hTUCcHYGUU8_3TL1U0mtZYOg'
if not SPREADSHEET_ID:
    print("Ошибка: Не найден ID Google Sheet в переменных окружения.")
    exit()

image_folder = '/home/vlados/bot_for_im_in/photos_for_bot'
...
questions = {
    1: {
        'question': '*Ты - организатор мастер-класса «Я в деле». Чем займешься?*',
        'options': {
    ...
    2: {
        'question': '*Выбери утверждения, которые описывают тебя*',
        'options': {
    ...
    4: {
        'question': '*Какое высказывание лучше отражает твою жизненную позицию?*',
        'options': {
            'А': 'Дьявол в мелочах',
    ...
    5: {
        'question': '*Какая картинка тебе больше всего нравится?*',
        'options': {
            'А': 'Картинка 1',
            'Б': 'Картинка 2',
            'В': 'Картинка 3',
...
user_steps = {}
department_images = {
    'М': '/home/vlados/bot_for_im_in/methodic.png',
    'HR': '/home/vlados/bot_for_im_in/hr.png',
    'И': '/home/vlados/bot_for_im_in/event.png',
    'Н': '/home/vlados/bot_for_im_in/admin.png',
    'П': '/home/vlados/bot_for_im_in/partner.png',
    'А': '/home/vlados/bot_for_im_in/nabor.png'
}
...
def send_question(chat_id, question_number):
    ...
    bot.send_message(chat_id, "Выбери ответ:", reply_markup=create_markup(question_number))
...
def ask_fio(call):
    chat_id = call.message.chat.id
    bot.send_message(chat_id, "Пожалуйста, введи свое ФИО:")
...
def send_department_result(chat_id, department_code):
    ...
    if department_code in department_descriptions:
        ...
        if 'description' not in department_info:
            logging.error(f"Description not found for department code: {department_code}")
            bot.send_message(chat_id, "Описание отдела не найдено.")
            return
        ...
        if department_code in department_images:
            ...
            logging.info(f"Image path found: {image_path}")
            try:
                """Попытка открытия и отправки фотографии"""
                logging.info(f"Trying to open image: {image_path}")
                with open(image_path, 'rb') as photo:
                    logging.info(f"Image opened successfully. Sending photo...")
                    bot.send_photo(chat_id, photo, caption=description, parse_mode="Markdown")
                    logging.info(f"Photo sent successfully.")
            except FileNotFoundError:
                logging.error(f"Image file not found: {image_path}")
                bot.send_message(chat_id, description, parse_mode="Markdown")
                logging.info(f"Department description sent without image.")
            except Exception as e:
                logging.error(f"Error sending department result with image: {e}")
                bot.send_message(chat_id, description, parse_mode="Markdown")
                logging.info(f"Department description sent without image.")
        else:
            ...
            try:
                bot.send_message(chat_id, description, parse_mode="Markdown")
                logging.info(f"Department description sent successfully.")
            except Exception as e:
                logging.error(f"Error sending department description: {e}")
    else:
        logging.error(f"Department code not found: {department_code}")
        try:
